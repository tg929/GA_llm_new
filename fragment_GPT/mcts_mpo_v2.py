...
只访问可扩展结点
n_total_children = 10
iter.10000 AUC for Best 10 Mean: 0.4243
iter.10000 AUC for Best 100 Mean: 0.3400
'''
...
class MCTSConfig:
    ...
    init_children = 15  # initial number of children to expand at the root node. if -1, use N_TOTAL_CHILDREN. This is a piror knowledge of the problem.
    n_total_children = 15  # number of children to expand at each node
...
class MonteCarloTreeSearchNode:
    ...
    def update_n_total_children(self, increase_factor):
        ...
        new_n_total_children = min(int(increase_factor * value_diff), 20)
...
class MCTS:
    ...
    def run_mcts(self):
        ...
        self.save_tree('./tree_log/root_v2.p')
        # 使用 'w' 模式写入文件，确保指定编码以支持中文
        with open("./tree_log/buffer_v2.json", "w", encoding="utf-8") as f:
