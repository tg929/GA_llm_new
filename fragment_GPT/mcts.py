...
class MolecularProblemState:
    ...
    def actions(self, is_greedy=False):
        ...
        """
        temperature = 0.0 if is_greedy else 0.8
...
class MonteCarloTreeSearchNode:
    ...
    def add_simulate(self, n_simulations):
        ...
        for _ in range(n_simulations):
            # 假设 state.fast_rollout_evaluation() 里实现了：
            #  a) 随机或短步地生成新的分子/SMILES
            #  b) 对其进行打分(如QED)
            #  c) 返回这次“模拟”生成分子的评分
            reward_sample = self.fast_rollout_evaluation()

            reward_list.append(reward_sample)

        # 此处示例返回平均值
        return np.mean(reward_list) if len(reward_list) > 0 else 0.0
    ...
    def best_action_global_leaf(self):
        """
        找到整棵子树中全局平均 Q 值最高的叶子节点。
        """
        if not self.children:
            return self  # 自己就是叶子
        best_leaf = None
        highest_avg_q = float('-inf')
        for child in self.children:
            leaf = child.best_action_global_leaf()
            leaf_avg_q = leaf.q() / leaf.n() if leaf.n() > 0 else 0
            if leaf_avg_q > highest_avg_q:
                highest_avg_q = leaf_avg_q
                best_leaf = leaf
        return best_leaf
...
class MCTS:
    ...
    def run_mcts(self):
            ...
            if is_expand:
                ...
                if self.config.n_simulations > 0 and self.config.fastrollout_weight > 0:
                    if v.is_terminal_node():
                        ...
                        reward += self.config.fastrollout_weight * v.add_value(is_additional=False)
                    else:
                        # 否则做一次快速模拟
                        reward += self.config.fastrollout_weight * v.add_simulate(self.config.n_simulations)

                # 缓存这个 reward
                v.set_cached_reward(rv, rq, rs, raw_value)
                # 回溯更新
        ...
        # 最后拿一个“贪心”路径作为 final
        rv, rq, rs, smi, cur_sentence = self.root.best_action_greedy()
