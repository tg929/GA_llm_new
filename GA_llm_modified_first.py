...
# -*- coding: utf-8 -*-
"""
GA_llm_modified_first.py - 基于NSGA-II帕累托多目标优化的分子进化与生成流程 (改进版)
...
- 支持多受体并行对接，调用 operations/docking/docking_utils_demo.py
- 每个受体创建独立的target_*目录结构

=============================================================================
使用示例：
=============================================================================

# 基本使用（为所有10种受体创建独立的target_*目录）
python GA_llm_modified_first.py

# 指定特定的受体蛋白子集
python GA_llm_modified_first.py --targets 4r6e 3pbl 1iep

# 并行处理所有受体
python GA_llm_modified_first.py --parallel --max_workers 4
...
def run_gpt_generation(input_file, output_prefix, gen_num, logger, current_gen_output_dir):
    ...
    os.makedirs(default_gpt_script_output_dir, exist_ok=True)
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='GA_llm_modified_first - 基于NSGA-II帕累托多目标优化的分子进化与生成流程 (改进版)')
    ...
    parser.add_argument('--output_dir', type=str, default=os.path.join(PROJECT_ROOT, 'output_modified_first'),
...
4. 指定进化代数：
   python GA_llm_modified_first.py --generations 10
...
- decompose_results/: 分子分解的中间文件
- fragment_GPT_output/: GPT生成的中间文件
- multi_receptor_docking/: 包含各个受体的详细对接结果

关键改进：
- 独立目录结构：每个受体有独立的target_*目录，避免结果混淆
- 独立中间文件：分解和GPT生成的文件都保存在各代的独立子目录中
- 并行处理支持：可以并行处理多个受体的进化过程
- NSGA-II帕累托选择：同时优化对接分数、QED分数和SA分数
